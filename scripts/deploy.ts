...
const main = async () => {
  ...
  const CLAIMER = "0x978eb4bef0a31f9e582f194a72cff24f0d6cd821";

  await helpers.impersonateAccount(CLAIMER);
  const impersonatedSigner = await ethers.getSigner(CLAIMER);

  console.log("....  Sending ether to impersonator's address ...");
  await helpers.setBalance(CLAIMER, ethers.utils.parseEther("2000000"));

  const signerBal = await impersonatedSigner.getBalance();
  console.log("Signer ETH balance", signerBal);
  ...
  console.log("... Verifying signer's address and claiming airdrop ...");
  
  let verified = await whitelist.connect(impersonatedSigner).checkInWhitelist(proof, 100);
  let addressBal = await whitelist.balanceOf(impersonatedSigner.address);

  console.log("Address balance", addressBal);


  let claimed = await whitelist.checkClaimed(impersonatedSigner.address);
